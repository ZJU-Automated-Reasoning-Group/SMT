...
#ifndef SMTLIBSOLVER_EXCEPTIONS_H_
#define SMTLIBSOLVER_EXCEPTIONS_H_
...
public:
	 ...
	 *                 with the caller.
	 */
	explicit SmtException(const char *message) :
			msg(message) {
	}
	 ...
	 */
	explicit SmtException(const std::string &message) :
			msg(message) {
	}
...
public:
	explicit IncorrectUsageException(const char *msg) :
			SmtException(msg) {
	}
	;
...
public:
	explicit NotImplementedException(const char *msg) :
			SmtException(msg) {
	}
	;
...
public:
	explicit InternalSolverException(const char *msg) :
			SmtException(msg) {
	}
	;
...
#endif /* SMTLIBSOLVER_EXCEPTIONS_H_ */
