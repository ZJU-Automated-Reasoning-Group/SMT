...
int main(int argc, char **argv) {
	...
	std::set<std::pair<pid_t, int>> FreeMSQs;
	...
	while (true) {
		...
		try {
			...
			if (M == std::string::npos || M + 1 == Command.length()) {
			...
			if (UserRequest == "open") {
				...
				if (It == UserWorkerMap.end()) {
					if (!FreeMSQs.empty()) {
						// try reuse
						auto It = FreeMSQs.begin();
						DEBUG(errs() << "[Master] (open) Reusing and sending to " << UserId << ": " << It->second << "...");
						if (CommunicateMSQ->sendMessage(std::to_string(It->second), UserId) == -1) {
							throw std::runtime_error("[Master] Fail to send slave id to user after open with reuse!");
						}
						DEBUG(errs() << "Done!\n");

						UserWorkerMap[UserId] = *It;
						FreeMSQs.erase(It);
				...
				if (It != UserWorkerMap.end()) {
					...
					if (!FreeMSQs.empty()) {
						auto It = FreeMSQs.begin();
						DEBUG(errs() << "[Master] (reopen) Reusing and sending to " << UserId << ": " << It->second << "...");
						if (CommunicateMSQ->sendMessage(std::to_string(It->second), UserId) == -1) {
